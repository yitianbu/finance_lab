...
MA5_EXIT_CHECK_SESSIONS = 5
MA5_EXIT_MAX_HOLD_SESSIONS = 15
QUOTE_FIELDS = ",".join([
...
def planned_trading_date(trade_date: str, sessions: int) -> str:
...
def planned_sell_date(trade_date: str) -> str:
    return planned_trading_date(trade_date, MA5_EXIT_CHECK_SESSIONS)


def ma5_exit_plan(trade_date: str) -> dict[str, Any]:
    return {
        "check_date": planned_trading_date(trade_date, MA5_EXIT_CHECK_SESSIONS),
        "max_exit_date": planned_trading_date(trade_date, MA5_EXIT_MAX_HOLD_SESSIONS),
        "check_sessions": MA5_EXIT_CHECK_SESSIONS,
        "max_hold_sessions": MA5_EXIT_MAX_HOLD_SESSIONS,
        "rule": "第5个交易日检查：若净收益不为正或收盘跌破MA5则卖出；若盈利且站上MA5则继续持有，之后跌破MA5或达到最长持有日卖出。",
    }


def ma5_exit_decision(
    *,
    entry_price: float,
    close: float,
    ma5: float,
    ...
    check_sessions: int = MA5_EXIT_CHECK_SESSIONS,
    max_hold_sessions: int = MA5_EXIT_MAX_HOLD_SESSIONS,
) -> dict[str, Any]:
    if entry_price <= 0 or close <= 0 or ma5 <= 0:
        return {"action": "wait", "reason": "insufficient_price_data", "net_return": 0.0}
    net_return = safe_div(close, entry_price, 1.0) - 1.0 - round_trip_cost
    if held_sessions < check_sessions:
        return {"action": "hold", "reason": "before_day5_check", "net_return": net_return}
    if net_return <= 0:
        return {"action": "sell", "reason": "day5_net_return_not_positive", "net_return": net_return}
    if held_sessions >= max_hold_sessions:
        return {"action": "sell", "reason": "max_hold_sessions", "net_return": net_return}
    if close < ma5:
        return {"action": "sell", "reason": "close_below_ma5", "net_return": net_return}
    return {"action": "hold", "reason": "profit_above_ma5", "net_return": net_return}
...
def write_outputs(output_dir: Path, rows: list[dict[str, Any]], errors: list[dict[str, str]], meta: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"若净收益不为正或收盘跌破MA5则卖出，若盈利且站上MA5则继续持有；"
        f"最长持有至 {meta['max_exit_date']}（第{meta['max_hold_sessions']}个交易日）。",
        "",
        "## 结论",
        conclusion,
        "",
        "## 风险诊断",
        f"- 诊断状态：{switch.get('state', '未评估')}（仅作风险提示，不再作为开仓硬拦截）。",
        f"- 风险原因：{'; '.join(switch.get('reasons') or ['无明显风险原因'])}",
...
def main() -> None:
    ...
    try:
        ...
        indices = {}
    exit_plan = ma5_exit_plan(latest_date)
    meta = {
        "generated_at": generated_at,
        "latest_date": latest_date,
        "sell_date": exit_plan["check_date"],
        "exit_check_date": exit_plan["check_date"],
        "max_exit_date": exit_plan["max_exit_date"],
        "exit_check_sessions": exit_plan["check_sessions"],
        "max_hold_sessions": exit_plan["max_hold_sessions"],
        "exit_rule": exit_plan["rule"],
    ...
    attack_mode = evaluate_attack_mode(rows, recent_metrics)
    ...
    if recent_metrics is None:
        meta["recent_metrics_note"] = "未接入最近已完成信号滚动统计，仅使用市场/候选/尾盘风险诊断。"
    else:
        optimized_gate_note = ""
        if {"medium_excess_best_return", "medium_max_drawdown"}.issubset(recent_metrics.keys()):
            optimized_gate_note = (
                ...
                f"强收益状态参考要求近{BENCHMARK_GATE_SAMPLE_DAYS}批中至少{BENCHMARK_GATE_MIN_SAMPLES}批、"
            ...
            optimized_gate_note = "强收益状态参考缺少最强指数超额/中期回撤字段，未评估。"
